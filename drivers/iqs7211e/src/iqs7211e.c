...
static void iqs7211e_emit_click(struct iqs7211e_data *data, uint16_t code) {
    if (data->pending_click_code != 0) {
        (void)k_work_cancel_delayable(&data->click_work);
        input_report_key(data->dev, data->pending_click_code, 0, true, K_FOREVER);
    }

    input_report_key(data->dev, code, 1, true, K_FOREVER);
    data->pending_click_code = code;
    (void)k_work_reschedule(&data->click_work, K_MSEC(IQS7211E_CLICK_MS));
}
